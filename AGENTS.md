...
**ブラウザ標準のフォーカスリング（青い囲い）も同じ方針で無効化している**（`index.css` の `:focus, :focus-visible { outline: none }`。`:focus` は Safari、`:focus-visible` は Chrome / Firefox 向けで両方要る）。`useOverlayA11y` がオーバーレイを開いた瞬間に内部の最初の操作要素へフォーカスを移すため、タップで開いただけでドロワーの「大学ホームページ」や設定の「戻る」に青枠が出て、押した覚えのないものが選択されて見えていた。**これはアクセシビリティ上の見落としではなく意図的な削除なので、「フォーカスリングが無い」ことを理由に戻さない。** フォーカスの移動自体（初期フォーカス・復帰・`inert` による Tab 順制御）は変えていないので、消えたのは見た目のリングだけ。入力欄を持たないアプリなので実質の影響は PC でキーボード Tab 移動をしたときの現在位置表示に限られる。復活させるなら `:focus-visible` にテーマ色の控えめなリングを定義する（タップ由来のフォーカスには出ない）。

**セクションカードの縁と影:** ホームの各カード（次のバス・今後の発車時刻・全時刻表・乗り場マップ・エラー/日付変更カード）は `index.css` の `.section-card`（`--bg-card` のべた塗り面）と `.hero-card`（グラデーションの主役カード）の 2 クラスで縁を表現する。**影の値をコンポーネントに直書きしない** — 単一の真実源は CSS 側で、JSX が渡すのは `.hero-card` の影の色 `--hero-tint`（グラデ始点＝暗い側の RGB。`RouteToggle` の `--toggle-tint` と同じ分業）だけ。角丸はカードごとに 20px / 22px と異なるためクラスに含めず各要素に残している。落ち影（`--card-shadow`）は光源を左上に置いて右下へ回し、ダークは地に沈むぶん強くしてある。`.hero-card` は輪郭を作る近距離層を黒に寄せている（色を帯びた近距離影は発光に見えるため）。

境界線（`--card-border`）と縁の稜線（`--card-rim`）は **`border` ではなく `::after` の inset シャドウで描く**。inset は要素の背景の上・**子要素の下**に描かれるので、中身が端まで敷き詰まるカード（乗り場マップ = Leaflet が全面を覆う）では border/inset の稜線がタイルに隠れて消えるため。カード側に `border` を持たせないことで、`::after` の `inset: 0`（containing block = padding box）と `border-radius: inherit` が要素の外形とぴったり一致する。稜線の向きは落ち影と同じ左上光源で、ライトはカード面が白なので右下を沈ませる側だけ、ダークは上左に光のリムを置ける。

**両クラスが持つ `position: relative` は重なり順のためで、外すと影が隣のカードに乗る。** 次のバスカードは装飾の円を置くため元から positioned で、positioned 要素は後続の非 positioned なカードより手前に描かれる。そのままだと下向きの影が「今後の発車時刻」カードの上端に被るので、全カードを positioned にして DOM 順どおり「後のカードが手前」に戻している。`z-index` は `auto` のままなので、オーバーレイ（ドロワー 30・各画面 50・PWA 案内 100・バナー 110・Toast 200）との重なりには影響しない。
