...
| 未確定な掲示 | 時刻を作らない。期間が安全に読める場合だけ特別ダイヤにする。前回の特別ダイヤは、同じ掲示が有効なデータとして取り込めるか掲示が消えるまで維持する（別種別に分類し直されて取り込みに失敗した回、リンクを 1 件も抽出できなかった回は消さない） |
| 非実在日・逆転期間 | 期間を残さず `needs_review` にする。特別ダイヤでは塗らない |
| 消えた未来イベント | 健全な抽出が 3 回連続で欠落した場合だけ撤去候補にする |
| 消えた通常ダイヤリンク | `warn` として必ずメールに載せる。既存ファイルは変更しない。ただし長期休暇の期間中（終了日が確定している休暇に今日が含まれる間）は掲示が外れるのが通常なので `info` に落とし、参考情報として残す |
...
| 便数の急変 | 既存ファイルの更新で便数が ±50% を超えて変わったら、そのファイルを書かない。正しい改正なら時刻表 JSON を手動で更新して復旧する（比較対象は既存ファイルの便数なので `bot/state.json` のキー削除では解除されない） |
...
OCR は通常 **`gemini-3.8-flash`** を使います。1 リクエストが 180 秒で `AbortError` / `TimeoutError` になった場合は、3.8 を再試行せず **`gemini-3.7-flash`** へ即時に切り替え、その実行中は 3.7 を使い続けます。モデル利用不可（403 / 404）、RPD 枯渇と判定した 429、または再試行後も続く一時障害も 3.7 への切替対象です。通常の RPM 429 と 503 等の一時障害はモデルを替えずバックオフ再試行を優先します。両モデルの実際の RPM / RPD はプロジェクトごとに AI Studio で確認し、確認できるまでは上記の保守的な上限を維持します。

通知メールへ連結する実行レポートは、Gemini を実際に 1 回以上呼んだときだけ最終使用モデルを表示します。`SKIP_OCR=1` や予算・締切により OCR を呼ばなかった実行では、モデル名を表示しません。

レポートはワークフローが Markdown から HTML に変換してメールにするため、掲載ページ由来の文字列（行テキスト・画像 URL・警告文・OCR のラベル）は `bot/src/report.ts` で HTML の `& < >` と Markdown のリンク記法・強調記法をエスケープしてから埋めます。変換器は raw HTML を素通しするので、素のまま埋めると外部からメール本文の構造・リンク先・画像を変えられます（2026-09-12、Codex レビュー SEC-20260912-02）。時刻表 ID・ファイル名の `_` も同じ関数で逃がします。変換器は単語の途中の `_` も強調として扱うため、`timetable_weekday / timetable_holiday` が「timetableweekday / timetableholiday」と表示されるメールが届いていました（2026-09-12 に受信メールで確認）。
