...
3タブ構成への改修（2026-09）で、PC / 横向き専用の2カラム表示（旧 `html.bp-active`）は一度廃止しました。その後、PC画面向けに `1024px` 以上で「サイドバー＋中央寄せ単一カラム」を再導入しています（複数カラムのダッシュボード化はしない。詳細は [design-decisions.md](design-decisions.md#レスポンシブの境界) を参照）。1024px未満は変わらず縦1カラムのタブ構成です。
...
| コンポーネント | 主な責務 |
|---|---|
| `BottomTabBar` | バス／マップ／メニューの固定3タブ。選択表示は色のみ。`position: fixed` で viewport 下端に常時表示する（各タブ本文は `--tabbar-h` ぶんの下パディングでスペースを確保する）。1024px以上は `Sidebar` に置き換わり非表示になる |
| `Sidebar` | 1024px以上でボトムタブバーの代わりに表示する左サイドバー（PC専用）。`BottomTabBar` と同じ `activeTab` を読み書きするだけで、独自の状態は持たない |
