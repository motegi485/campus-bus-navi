...
- Google マップの埋め込みによる乗り場の地図と乗り場の様子（ストリートビュー）の表示、Google マップでの徒歩ルート案内
- お知らせ、テーマ、文字サイズ、初期表示路線の設定
- 当日の便を選んでの発車前通知（アプリを閉じていても届きます）
- 一度取得した時刻表・お知らせのオフライン表示（地図とストリートビューはオフラインでは表示されません）

## 使い方

- 画面下の「バス」「マップ」「メニュー」の 3 つのタブで切り替えます（PC では左のサイドバー）。「メニュー」タブから、お知らせ、週間ダイヤ、設定、ヘルプ、アプリの初期化を開けます。
- 上部の切替で「大学発」と「松永発」を選べます。
- ホーム上部の日付（例: 9/12（土））をタップするか、メニューの「週間ダイヤ」から、今日を含む 7 日間の運行予定を確認できます。日を選ぶとその日の発車時刻が出ます。
