...
**セクションカードの縁と影:** ホームの各カード（次のバス・今後の発車時刻・全時刻表・乗り場マップ・エラー/日付変更カード）は `index.css` の `.section-card`（`--bg-card` のべた塗り面。`--card-border` のヘアライン + `--card-shadow`）と `.hero-card`（グラデーションの主役カード）の 2 クラスで縁を表現する。**影の値をコンポーネントに直書きしない** — 単一の真実源は CSS 側で、JSX が渡すのは `.hero-card` の影の色 `--hero-tint`（グラデ始点＝暗い側の RGB。`RouteToggle` の `--toggle-tint` と同じ分業）だけ。角丸はカードごとに 20px / 22px と異なるためクラスに含めず各要素に残している。ダークは落ち影が地に沈むので、`--card-shadow` に上端の内側リムを足し、`.hero-card` は輪郭を作る近距離層を黒に寄せている（色を帯びた近距離影は発光に見えるため）。
