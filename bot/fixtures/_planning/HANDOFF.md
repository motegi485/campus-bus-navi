...
> **2026-09-03〜04 の OCR モデル更新**: ユーザー承認により primary を `gemini-3.8-flash`、障害時 fallback を `gemini-3.7-flash` へ更新した。2026-09-04 に代表画像で 3.8 の実 OCR を試行したところ、既存の 180 秒リクエストタイムアウトが 2 回連続で発生した。このため primary の `AbortError` / `TimeoutError` は再試行せず 3.7 へ即時切替し、同一実行では 3.7 を固定するよう変更した。通常の RPM 429 と 503 等の一時障害は既存どおりバックオフ再試行を優先する。モデル利用不可・RPD 枯渇・早期タイムアウト・503 の切替経路をモックした回帰テストを追加した。通知メールのモデル行は、Gemini を一度も呼ばなかった実行では省略する。**3.8 / 3.7 の実 API 互換性・精度は未確認。** AI Studio で対象プロジェクトの RPM / RPD と API ログを確認してから、代表画像 2 枚を両モデルで実際に OCR するまでは、18 回/実行・日を保守的に維持する。
...
### 2026-09-12 の変更（Codex レビュー第 2 回の反映・要件定義 v1.15）

`_codexReview/CODEX_REVIEW_2026-09-12.md` のうち、実装で再現・確認できた Bot 関連 3 件を反映した。
運用に関わるのは次の 3 点。ローカルで Bot テスト 209 件と型検査が合格している（実ネットワーク・OCR・秘密情報は未使用）。
GitHub Actions 上の実行結果は未確認。Secrets や Workflow permissions の変更は不要。

- **特別ダイヤは「掲示が読めるようになるまで」残る。** 前回 `needs_review` だった掲示が同じ URL のまま
  「夏季休業」などに分類し直され、その画像の取得・OCR・検証に失敗した回は、前回の特別ダイヤを維持して
  `⚠ 要手動確認` に「掲示は残っていますが取り込めませんでした」と出る（以前は保護だけが消えて通常ダイヤ表示に戻っていた）。
  ページからリンクを 1 件も抽出できなかった回も維持する（参考情報に出る）。掲示が消えた回と、取り込みに成功した回は従来どおり外れる。
- **便数 ±50% ガードの復旧手順を訂正した。** 上の切り分け表のとおり、`bot/state.json` のキー削除では解除されない。
  正しい改正なら時刻表 JSON を手動で更新する。
- **通知メールの本文で外部由来の文字列を HTML エスケープするようにした。** 掲載ページの行テキストや URL に
  `<` `>` `&` `[` `]` が含まれると、メール上では `&lt;` などのエンティティとして書かれた Markdown になる
  （HTML メールとして表示されれば元の文字に見える）。見た目の変化はそれだけで、判断基準は変わらない。
  併せて、時刻表 ID・ファイル名の `_` も `\_` で逃がす。それまでのメールでは `timetable_weekday / timetable_holiday`
  が「timetableweekday / timetableholiday」と `_` を失って届いていた（HTML 変換が単語途中の `_` を斜体にしていた）。
- **長期休暇中の「⚠ 要確認」メールをやめた。** 長期休暇中は通常ダイヤの掲示が外れるのが通常（ユーザー説明、2026-09-12）
  なのに、`regular_link_missing` が毎日 warn で出て差分ゼロの日も毎日メールが届いていた（2026-08-17 以降、受信メールで確認）。
  休暇期間（state か今回の掲示の期間で、終了日が確定しているもの）に今日が含まれる間だけ info に落とす。レポートの
  「参考情報」には残る。休暇期間外に消えた場合は従来どおり warn で届く。**次の人間確認**: この変更を `main` へ反映した後、
  休暇中の差分ゼロの日にメールが届かないこと（月初 1 日の稼働確認は例外）と、9/24 以降に通常ダイヤの掲示が戻ることを確認する。
  戻らなければ warn のメールが届くので、掲示の文言変更を疑う。
...
- **AC-7**: `bot/state.json` に登録済みの 2026-08-23 オープンキャンパスについて、
  ...
  自動削除され、その旨のメールが届くこと。
- **H-8**: AI Studio で `gemini-3.8-flash` と `gemini-3.7-flash` の無料枠実値を確認し、必要なら
  `geminiMinIntervalMs` / `geminiMaxCallsPerRun` を調整する。3.8 の実 OCR は 180 秒タイムアウトが
  2 回続いて中止しているため、両モデルの代表画像による照合もここで実施する。
...
| 便数が変わらないのに取り込まれない | 便数 ±50% ガード（FR-8）で止まっている可能性。メールの「要手動確認」を読む。正しい改正なら**元画像を確認して時刻表 JSON を手動で更新する**。更新後は次回実行の比較基準が新しい便数になるので、Bot が同じ画像を読み直しても止まらない。**`bot/state.json` のキー削除では解除されない**（比較対象は state ではなく既存の時刻表ファイル。2026-09-12 訂正） |
