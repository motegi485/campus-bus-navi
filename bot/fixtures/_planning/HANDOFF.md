...
- **AC-4（変更が無い日にコミットもメールも発生しないこと）— 既知の問題として未解決。**
  `2026-08-19`〜`2026-09-01` の 13 日間、**無変更の日が 1 日も無く**、
  `public/data/calendar_rules.json` が毎日 1〜2 行ずつ減っている（期限切れ override の剪定と見られる）。
  `Detect diff` ステップは `public/data/` 配下の差分を機械的に見ているため、この剪定だけでも
  「時刻表を更新しました」メールが毎日送られている可能性が高い。**今回のセッションでは対処しない。
  別セッションで、剪定のみの日を「無変更」として扱うかどうかを検討すること。**
