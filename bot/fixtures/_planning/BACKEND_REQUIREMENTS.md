...
| 項目 | 内容 |
|---|---|
| 文書バージョン | 1.15（Codex レビュー 2026-09-12 の反映: 特別ダイヤの維持・便数ガード復旧手順・メール本文のエスケープ／休暇中の通常ダイヤ欠落を info に） |
| v1.15 変更点 | 2026-09-12、Codex による全体レビュー（`_codexReview/CODEX_REVIEW_2026-09-12.md`）のうち、実装で再現・確認できた Bot 関連 3 件を反映。併せて **FR-2 6(b)**: 長期休暇の期間中は通常ダイヤの掲示が外れるのが通常（ユーザー説明）なので、その間の `regular_link_missing` を `warn` から `info` に落とし、休暇中に毎日「⚠ 要確認」メールが届くのをやめる（休暇期間外は従来どおり warn）。**FR-9（FN-20260912-01）**: `state.specials` を「今回の `needs_review` 集合」で丸ごと置き換える実装だったため、前回 `needs_review` だった掲示が同じ URL のまま別種別（夏季休業など）に分類し直され、その画像の取得・OCR・検証に失敗した回に、有効なデータが無いのに特別ダイヤの保護だけが消えて通常/休暇ダイヤ表示へ戻っていた。リンクを 1 件も抽出できなかった回も同じ経路で全消去されていた。前回のエントリを掲示単位で判定し、**掲示が残っていて取り込みに失敗した回は維持（`warn`）、抽出失敗の回は維持（`info`）、掲示が消えた回と有効なデータに置き換わった回は従来どおり削除**する。`buildPlan` に `presentUrls`（今回抽出した全リンクの URL）を追加。**FR-8（OPS-20260912-02）**: 便数 ±50% ガードの復旧手順「`bot/state.json` の該当キーを削除して再実行」は、比較対象（`prevCounts`）を state ではなく `public/data` の既存ファイルから作るため解除にならない。復旧は「元画像を確認して時刻表 JSON を手動で更新する」に統一し、エラー文・HANDOFF・docs を訂正。**FR-11（SEC-20260912-02）**: レポートは `convert_markdown: true` で HTML メールになるが、掲載ページ由来の文字列（cheerio の `.text()` でエンティティ復号済み・`decodeURIComponent` 済み URL・OCR ラベル）を HTML エスケープしていなかった。外部由来の文字列は `& < >` と Markdown リンク記法の `[ ]` を逃がしてから本文へ埋める。回帰テストを `plan.test.ts` / `report.test.ts` に追加。 |
...
3. **事後確認の担保（v1.9 で改訂、v1.11 で通知対象を明確化）**: 反映は自動だが、**利用者に影響する削除を含む全変更を必ず通知メールに載せる**。書かなかった・消さなかった判断（要手動確認）も同じメールに出す。「黙って反映する」「黙って止まる」のどちらも禁止。ただし、今日以降の表示に影響しない過去日の管理 override 剪定だけは更新通知から除く。旧 v1.8 の「反映は必ず PR 経由」はこの項に置き換わった。
...
- **【v1.9 で MUST へ格上げ】既存ファイル更新時、便数変化が ±50% 超ならそのファイルを書かない。**
  ...
  正しい改正でここに落ちた場合の復旧は**「元画像を確認して時刻表 JSON を手動で更新する」**。手動更新後は
  既存ファイルの便数が新しい値になるので、次回の実行で同じ画像を読み直しても比較の基準が更新後の便数になり、
  ガードには掛からない（OCR が通れば state も同じ実行で記録される）。この手順は通知メールにも書かれる
  （`validate.ts` のエラーメッセージ）。**【v1.15】`bot/state.json` の該当キー削除は解除手段にならない。**
  比較対象（`prevCounts`）は state ではなく `public/data` の既存ファイルから作る（`plan.ts`）ため、
  state を消しても「新規画像」として OCR し直したうえで同じガードに再び掛かる（v1.14 まではこの誤った
  手順を案内していた。Codex レビュー OPS-20260912-02）。
...
- 【v1.15】**前回の `state.specials` の引き継ぎ**（Codex レビュー FN-20260912-01）。今回の `needs_review` だけで
  丸ごと置き換えると、前回 `needs_review` だった掲示が同じ URL のまま別種別（vacation / regular / event）に分類し直され、
  その画像の取得・OCR・検証に失敗した回に、有効なデータは無いのに保護だけが消える。前回の各エントリを次の順で判定する:
  1. 今回も同じ URL が `needs_review` にある → 今回の結果で置き換える（期間が変わっても旧期間を残さない）
  2. 期間が終わっている → 捨てる
  3. ページから時刻表リンクを 1 件も抽出できなかった回（`extractionHealthy = false`）→ 判定できないので維持し `info`
     （`special_kept_unverified`。FR-4 の event 撤去と同じ扱い）
  4. 掲示がページから消えた（`presentUrls` に無い）→ 捨てる（従来どおり）
  5. 同じ URL が有効なデータとして取り込めた（OCR 成功・画像同一など）→ 置き換わったので捨てる
  6. それ以外（別種別になったが取り込みに失敗）→ 維持し `warn`（`special_kept_unreplaced`）で通知メールに載せる
  維持するときは `processed_at` を含めて前回値をそのまま残す（同じ日に 2 回実行しても state が変わらない）。
  `buildPlan` は `presentUrls`（今回抽出した全リンクの正規化 URL。分類・採否を問わない）を受け取り、
  `index.ts` が `classified` 全件から渡す。
...
2. 成功 → `bot/holidays.json` を `{ fetched_at, source_sha256, holidays:[...] }` で上書き（コミットに同梱）。
   **【v1.5 追加】ただし取得した CSV の SHA-256 が既存キャッシュと同じなら書き換えない。**
   毎回 `fetched_at` を更新すると、データ無変更の日でも不要な差分と commit が積まれ、NFR-1（冪等性）と AC-3 に反する。
...
- Bot 本体は **commit も push もしない**。作業ツリーにファイルを書き、`bot/.out/report.md` を生成し、
  `GITHUB_OUTPUT` に `has_warn` / `calendar_cleanup_only` / `report_written` を出すところまでが責務。
- ワークフローが次の順で適用する。
...
- `calendar_cleanup_only=true` は、計画上の実変更（`skip` を除く）がすべて `date < today` の override 削除である場合だけ出す。
  ワークフローはさらに、実際の `public/data` 差分が `calendar_rules.json` だけであることを確認する。
  両方が成立した場合も剪定・state 更新の commit は行うが、今日以降の表示内容を変えない定期保守なので更新メールは送らない。
  Bot の判定と実差分が食い違う場合、または他の public データが1件でも変わった場合は通知側へ倒す。

#### 11.2 通知
利用者に影響するデータ差分・警告・失敗のいずれかがある実行だけメールを送る。**過去日の override 剪定だけの日は送らない**（ユーザー決定）。

| 状況 | 送信 | 件名 |
|---|---|---|
| ジョブ失敗（取得失敗・トリップワイヤー・鍵なし・検証失敗・push 失敗） | ✔ | `❌ [campus-bus-navi] 自動取得に失敗しました (MM-DD)` |
| 通知対象の `public/data/**` 差分あり ＋ 要手動確認あり | ✔ | `⚠ [campus-bus-navi] 時刻表を更新しました／要確認あり (MM-DD)` |
| 通知対象の `public/data/**` 差分あり（要手動確認なし） | ✔ | `[campus-bus-navi] 時刻表を更新しました (MM-DD)` |
| 差分なし ＋ 要手動確認あり | ✔ | `⚠ [campus-bus-navi] 要確認 (MM-DD)` |
| 過去日の管理 override 剪定だけ、要手動確認なし | – | （commit と push はする） |
| `state.json` / `holidays.json` だけの差分、要手動確認なし | – | （コミットと push はする） |
| 完全に変化なし | – | |
| 毎月1日、上記の送信条件なし | ✔ | `[campus-bus-navi] 稼働確認 (MM-DD)` |
...
- 本文は `bot/.out/report.md` の先頭に実行日時・実行ログ URL・反映コミット URL を足したもの。
  ...
  （**無言で失敗させない**）。
- 【v1.15】本文は `convert_markdown: true` で HTML 化され、変換器（Showdown）は raw HTML を素通しする。
  レポートに埋める**外部由来の文字列**（掲載ページの行テキスト・画像 URL・警告文・OCR のラベル）は、
  用途別の Markdown エスケープ（v1.10: セルの `|`、リンク URL の括弧・空白）に加えて、
  HTML の `& < >` と Markdown リンク記法の `[ ]`、強調記法の `_ *` を逃がしてから埋める（`report.ts` の `escapeExternal`）。
  掲載ページのテキストは cheerio の `.text()` でエンティティが復号され、URL は `decodeURIComponent` 済みなので、
  素のまま埋めると外部からメール本文の構造・リンク先・リモート画像を変えられる（Codex レビュー SEC-20260912-02）。
  `_` は Bot 生成の時刻表 ID・ファイル名にも当てはめる。Showdown は GitHub の Markdown と違い単語の途中の `_` も
  強調として扱うため、`timetable_weekday / timetable_holiday` が「timetableweekday / timetableholiday」と `_` を
  失って届いていた（2026-09-12 受信メールで確認）。
  `formatWarning` 自体はログ・Step Summary 用に素のまま返し、本文へ埋める `buildReport` 側で逃がす。
...
| 2 | `Run sync`（`id: sync`）= `npx tsx src/index.ts` | 常時 | env に `GEMINI_API_KEY` と `DRY_RUN`。`GITHUB_OUTPUT` へ `has_warn` と `calendar_cleanup_only` を返す |
...
| 4 | `Detect diff`（`id: diff`） | `dry_run != true` | `git status --porcelain` を `public/data` / `bot/state.json` / `bot/holidays.json` に限定。`changed` と通知対象を表す `data_changed` を返す。`calendar_cleanup_only=true` かつ実 public 差分が `calendar_rules.json` だけなら後者を false にする |
...
20. `calendar`: 変更なし、今日・未来の削除、未来の追加、過去日の削除と未来の追加の混在を
    「過去日の override 剪定だけ」と誤判定しない。`skip` はファイルの実変更ではないため無視する。
21. `calendar`: 2026-09-01 から 2026-09-02 へ日付が進み、前日の vacation override だけが
    削除される実運用ケースを `calendar_cleanup_only` 相当として判定する。
...
| AC-4 | **【v1.11 で再定義】**大学サイト由来の時刻データに変更も要手動確認もなく、差分が state の鮮度情報と過去日の管理 override 剪定だけの日に Actions が走る | **更新メールは届かず**、ジョブは緑で終わる。運用メタデータと剪定の commit は作られてよい。ファイル差分自体が無ければ commit もしない | 🧪 ローカルの変更分類テストは合格。GitHub Actions 上の通知抑止は `main` 反映後の無変更日に要確認 |
...
| AC-7 | **イベント実走** | 適用日より前の実行で `timetable_event_{YYYYMMDD}.json` 生成＋`overrides` 追加がコミットされ通知に出る。適用日経過後の実行で当該 override とファイルの削除がコミットされ通知に出る。`timetable_closed.json`・`_examples/` 配下は常に無傷 | ✅ 2026-08-24 の実行で `timetable_event_20260823.json` と override の自動撤去を確認済み |
| AC-8 | **【v1.9 追加・v1.11 更新】通知** | 時刻表が変わった実行でメールが届き、本文に発車時刻・元画像リンク・反映コミット URL・取り消し手順が含まれる。要手動確認があれば件名に ⚠ が付く。ジョブ失敗時は ❌ のメールが届く。過去日の override 剪定だけで警告もない日は届かない | 🟡 更新・削除メールの到達は運用者確認済み。v1.11 の剪定のみ抑止は `main` 反映後の無変更日に要確認 |
...
| 2 | クリーンアップ動作の実地確認、STEP_SUMMARY、便数±50%警告、運用調整 | AC-6 / AC-7 | ✅ 実装・テスト済み。AC-7 は 2026-08-24 の実走で確認済み |
| 3 | **【v1.9】自動適用と通知**: `prBody.ts` → `report.ts` への改称と発車時刻節の追加、便数 ±50% の MUST 化、ワークフローの適用・通知ステップ、文書更新 | AC-4 / AC-7 / AC-8 | ✅ リポジトリ側は完了。自動適用・更新メール・イベント削除メールの実走を確認済み |
| 4 | **【v1.11】期限切れ override 剪定だけの日の更新メール抑止**: Bot の変更分類、ワークフローでの実差分との二重確認、回帰テスト、文書更新 | AC-4 / AC-8 | 🧪 ローカル検証済み。`main` 反映後の無変更日に Actions 実走確認が必要 |
...
### 17.4 実施済み（2026-09-01 に稼働中と確認。以下は当時「残作業」として書いた原文）
...
[人間] H-6 Cloudflare Pages のデプロイと実機を確認
        ↓
[人間] AC-4 の確認（実変更・警告がなく、過去日の override 剪定だけの日に
...
- **【v1.5 重要 / v1.6 で状況更新 / 2026-09-01 で確認完了】GitHub Actions の `schedule` / `workflow_dispatch` はデフォルトブランチにワークフローファイルがある場合のみ動作する。**
  この前提は main 統合（2026-08-02）で満たされた。その後 Actions 画面で Enable され、2026-09-01 時点で
  日次実行が稼働中であることをコミット履歴で確認した。
